...
import { columnsCreators } from './columns';
import { generateCreatorsData } from '@/constants';
...
export default function CreatorsDashboard () {
  ...
  const [allData, setAllData] = useState<Creator[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const generatedData = generateCreatorsData(100);
  ...
  const handleExportCSV = () => {
    exportToCSV(table, {
      fileName: 'selected_creators_export.csv',
  ...
  if (isLoading) {
    return (
      <div className='container mx-auto p-4'>
        <h2 className='text-2xl font-semibold mb-6'>Creators</h2>
        <DataLoadingSkeleton />
      </div>
    );
  }
    ...
    <div className='container mx-auto p-4'>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
        ...
        <TabsContent value={activeTab}>
          <div className='flex flex-row flex-wrap gap-2 bg-white items-center p-2 justify-between'>
            <div className='flex items-center gap-2'>
              <DataTableSearch
                placeholder='Search...'
                value={globalFilter ?? ''}
                onChange={event => setGlobalFilter(event.target.value)}
                className='max-w-sm w-[19.5rem] h-[2rem]'
              />
              <DataTableFilter
              ...
              />
              <DataTableFilter
                column={table.getColumn('totalEarnings')}
                title='Earnings'
                options={[
